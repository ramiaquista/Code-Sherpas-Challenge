...
get "/customers"
      ...
      expect(JSON.parse(response.body)).to eq([YAML.load(@customer_one.to_json),YAML.load(@customer_two.to_json),])
delete "/customers/#{@customer_one.id}"
      expect(response.status).to eq(200)
get "/customers"
      expect(JSON.parse(response.body)).to eq([YAML.load(@customer_two.to_json)])
    end
end